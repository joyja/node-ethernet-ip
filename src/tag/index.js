...
class Tag extends EventEmitter {
  ...
  set controller_value(newValue) {
    ...
    if (
      Buffer.isBuffer(newValue) &&
      Buffer.isBuffer(this.state.tag.controllerValue)
    ) {
      bufferCompare =
        Buffer.compare(newValue, this.state.tag.controllerValue) === 0;
    }
  ...
  static isValidTagname(tagname) {
    ...
    // check segments
    if (
      tagname.split(/[:.[\],]/).filter(segment => segment.length > 40).length >
      0
    )
